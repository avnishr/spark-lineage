...
class FileDumpLineageDispatcher(filename: String, permission: FsPermission, bufferSize: Int)
  ...
  override def send(plan: ExecutionPlan): Unit = {
    ...
    while (finalIdList.length > 0) {
      ...
      plan.operations.other.get.filter(x => x.name.get.equals("Project") && x.id.equals(id)).foreach(x => {
        ...
        x.params match {
          ...
          case _ => println("No Elements")

        }

        var finalProjList = projList.map(col => {
          if (col.startsWith("expr"))
             attrOrExprMap.get(col).getOrElse("Empty")
          else
            col
        })
    ...
    var finalStr:StringBuilder = new StringBuilder()
      lineage.foreach( item => {
      finalStr.append(attrIdColNameMap.get(item._2.fromColName).getOrElse(-1))
      finalStr.append(",")
      finalStr.append(item._2.inputTableOrFile.getOrElse(-1))
        finalStr.append(",")
      finalStr.append(attrIdColNameMap.get(item._2.columnDest).getOrElse(-1))
        finalStr.append(",")
      finalStr.append(item._2.outputTableOrFile)
        finalStr.append("\n")})

    println(finalStr)
   // persistToHadoopFs(finalStr.toString(),filename)
